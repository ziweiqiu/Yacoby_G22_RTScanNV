...
def fit_esr(freq, ampl, min_counts = .5, contrast_factor = 1.5, num_of_peaks = -1, strain_filtering=False,
            verbose = False):
    """
    Returns lorentzian fit parameters for a typical NV esr sweep, giving 4 or 6 parameters depending on if 1 or 2
    lorentzian dips are detected.
    ...
    if max(freq) < F0:
        ...
        try:
            if len(freq_peaks) == 2:
                ...
                [(0, 0,0, -np.inf, -np.inf, min(freq), min(freq)), (np.inf, np.inf,np.inf, 0, 0, max(freq), max(freq))])

                if len(fit) == 7 and num_of_peaks == 1:
                    if verbose:
    ...
    if not fit is None:
        if len(fit) == 7:
            if ((calc_esr_noise(freq, ampl, fit) * contrast_factor > np.abs(fit[3])) and (
                    calc_esr_noise(freq, ampl, fit) * contrast_factor > np.abs(fit[4]))):
                fit = None
            elif ((calc_esr_noise(freq, ampl, fit) * contrast_factor > min(np.abs(fit[3]), np.abs(fit[4]))) and (
                    calc_esr_noise(freq, ampl, fit) * contrast_factor < max(np.abs(fit[3]), np.abs(fit[4])))):
                if np.abs(fit[3]) > np.abs(fit[4]):
                    start_vals = [start_vals[0], start_vals[3], start_vals[5], start_vals[1]]
                else:
                    start_vals = [start_vals[0], start_vals[4], start_vals[6], start_vals[2]]
                try:
                    if num_of_peaks != 2:
                        fit = fit_lorentzian(freq, ampl, starting_params=start_vals,
                                             bounds=[(0, -np.inf, 0, 0), (np.inf, 0, np.inf, np.inf)])
                ...
                # if this detects a single peak far shifted, throw it out
                if not fit is None and np.abs(fit[3] - F0) > MAX_STRAIN:
                    fit = None
    ...
    if not fit is None:
        if (len(fit) == 7 and int(fit[1]) <= MIN_WIDTH):
            fit = None
        elif (len(fit) == 4 and int(fit[3]) <= MIN_WIDTH):
            fit = None
    if not fit is None:
        if (len(fit) == 7 and int(fit[1]) >= MAX_WIDTH):
            fit = None
