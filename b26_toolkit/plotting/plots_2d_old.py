...
def plot_magnet_sweep2D_Fluor(image_data, extent, axes_image, max_counts=-1, min_counts=-1, colorbar=None,
                              axes_labels=None, title=None, colorbar_name=None):
    """
...
def plot_fluorescence_new(image_data, extent, axes_image, aspect='auto', max_counts=-1, min_counts=-1, colorbar=None,
                          axes_labels=None, title='Confocal Image', colorbar_name='kcps', axis_off=False):
    """
    plots fluorescence data in a 2D plot
    Args:
        image_data: 2D - array
    ...
    if title is not None:
        axes_image.set_title(title, fontsize=9.5)
    if axis_off:
        # axes_image.axis('off')
