...
def plot_esr(axes, frequency, counts, fit_params=None, plot_marker_data='b', plot_marker_fit='r', linestyle='-',
    ...
    Args:
        ...
        frequency: the real mw frequency (array) in Hz
        counts: counts (array)
        plot_marker:  (str) plot marker
        D: zero-field splitting ~ 2.87 GHz
        gama: gyromagnetic ratio ~ 2.8 MHz/Gauss
        err: error bar
        LO: if using an IQ mixer, this is the LO frequency in Hz
    ...
    if fit_params is not None and len(fit_params) and fit_params[0] != -1:  # check if fit valid
        if len(fit_params) == 4:  # single peak
            ...
            title = 'ESR, mean counts: {:.2f}kcps\nf1={:0.2f}MHz, w1={:0.2f}MHz, f2={:0.2f}MHz, w2={:0.2f}MHz\n a1 = {:.2f} %, a2 = {:.2f} % \nLO={:0.3f}GHz, RF power: {:.2f}dBm, |B| ~ {:.2f} G, θ ~ {:.2f} deg'.format(
                avg_counts, (fit_params[5]-LO) / 1E6, fit_params[1] / 1E6, (fit_params[6]-LO) / 1E6, fit_params[2] / 1E6,fit_params[3] * 100,
                            fit_params[4] * 100, LO/1E9,  mw_power, float(B_mag), float(angle))
    else:
        title = 'ESR, mean counts: {:.2f}kcps\nLO={:0.3f}GHz, RF power: {:.2f}dBm'.format(avg_counts, LO/1E9, mw_power)

    # plotting
    freq_to_plot= (np.array(frequency) - LO) / 1E6 # IF frequency in MHz

    axes.errorbar(freq_to_plot[0:len1], counts[0:len1], counts_err[0:len1], linestyle=linestyle, marker=marker)
...
def update_pulse_plot(axis, pulse_collection, pulse_colors=None, pulse_tag=None):
    ...
    if set(instrument_names_old) != set(instrument_names):
        ...
        for pulse in pulse_collection:
            patch_list.append(
                patches.Rectangle((pulse.start_time, instrument_names.index(pulse.channel_id) - .25), pulse.duration,
                                  0.5,
                                  fc=pulse_colors.get(pulse.channel_id, 'b')))
...
def update_1d_simple(axis, times, counts_list, counts_err_list=None):
    ...
    if len(axis.lines) != len(
            counts_list):  # don't update the plot if the number of lines to plot isn't equal to the number of counts lists
