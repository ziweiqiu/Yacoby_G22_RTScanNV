...
from b26_toolkit.scripts.qm_scripts.ramsey import RamseySyncReadout, Ramsey_SingleTau
from b26_toolkit.scripts.optimize import optimize
from b26_toolkit.instruments import NI6733, NI6220, NI6210, Agilent33120A, YokogawaGS200
...
class ScanningRamseyLockIn(Script):
    """
        Perform DC sensing measurements (based on Ramsey LockedIn with AFM) and scan along a 1D line.
        One can also choose to do Rabi and fluorescence optimize every several points.
        - Ziwei Qiu 3/27/2020
    """
    _DEFAULT_SETTINGS = [
        ...
        Parameter('point_b',
                  [Parameter('x', 1, float, 'last x-coordinate [V]'),
                   Parameter('y', 0, float, 'last y-coordinate [V]')
                   ]),
        Parameter('num_points', 100, int, 'number of points for NV measurement'),
        Parameter('scan_speed', 0.02, [0.01, 0.02, 0.04, 0.06, 0.08, 0.1],
        ...
        Parameter('do_rabi',
                  [Parameter('on', False, bool, 'choose whether to do Rabi calibration'),
                   Parameter('frequency', 10, int, 'do the Rabi once per N points'),
                   Parameter('start_power', -10, float, 'choose the MW power at the first point'),
                   Parameter('end_power', -20, float, 'choose the MW power at the last point'),
                   ]),
        Parameter('do_optimize',
    ...
    _INSTRUMENTS = {'NI6733': NI6733, 'NI6220': NI6220, 'NI6210': NI6210, 'afg': Agilent33120A,
                    'yokogawa': YokogawaGS200}
    _SCRIPTS = {'trig_ramsey': RamseySyncReadout, 'rabi': RabiQM, 'optimize': optimize, 'set_scanner': SetScannerXY_gentle,
                'afm1d': AFM1D_qm, 'afm1d_before_after': AFM1D_qm}
    ...
    def do_dc_sensing(self, label=None, index=-1, verbose=True):
        # update the tag of pdd script
        if label is not None and index >= 0:
            self.scripts['trig_ramsey'].settings['tag'] = label + '_ind' + str(index)
        elif label is not None:
            self.scripts['trig_ramsey'].settings['tag'] = label
        elif index >= 0:
            self.scripts['trig_ramsey'].settings['tag'] = 'trig_ramsey' + '_ind' + str(index)
        else:
            self.scripts['trig_ramsey'].settings['tag'] = 'trig_ramsey'
        ...
        if self.settings['do_rabi']['on'] and index >= 0 and index % self.settings['do_rabi'][
            ...
            try:
                ...
                if pi_time >= 16 and pi_half_time >= 16 and three_pi_half_time >= 16:
                    self.scripts['trig_ramsey'].settings['mw_pulses']['mw_frequency'] = \
                        float(self.scripts['rabi'].settings['mw_pulses']['mw_frequency'])
                    self.scripts['trig_ramsey'].settings['mw_pulses']['mw_power'] = \
                        float(self.scripts['rabi'].settings['mw_pulses']['mw_power'])
                    self.scripts['trig_ramsey'].settings['mw_pulses']['IF_frequency'] = \
                        float(self.scripts['rabi'].settings['mw_pulses']['IF_frequency'])
                    self.scripts['trig_ramsey'].settings['mw_pulses']['IF_amp'] = \
                        float(self.scripts['rabi'].settings['mw_pulses']['IF_amp'])
                    self.scripts['trig_ramsey'].settings['mw_pulses']['pi_half_pulse_time'] = float(pi_half_time)
        ...
        if not self._abort:
            if verbose:
                print('==> Executing DC sensing based on AFM-synced Ramsey...')

            try:
                self.scripts['trig_ramsey'].settings['fit'] = True
                self.scripts['trig_ramsey'].run()
                [A_fit, A_fit_err, phi_fit, phi_fit_err, freq_fit, offset_fit] = self.scripts['trig_ramsey'].data[
                    'fits']
                A_fit = np.array([A_fit])
                A_fit_err = np.array([A_fit_err])
                phi_fit = np.array([phi_fit])
                phi_fit_err = np.array([phi_fit_err])
            except Exception as e:
                print('** ATTENTION in Running trig_ramsey **')
                print(e)
                return np.array([0]), np.array([0]), np.array([0]), np.array([0]), np.array([0])
            else:
                return A_fit, A_fit_err, phi_fit, phi_fit_err
    ...
    def _function(self):
        ...
        self.scripts['trig_ramsey'].settings['to_do'] = 'execution'
        if not self._abort:
            # The first point
            try:
                A_fit, A_fit_err, phi_fit, phi_fit_err = self.do_dc_sensing(index=0)
            except Exception as e:
                print('** ATTENTION do_dc_sensing (index=0) **')
                print(e)
            else:
                self.data['A_fit'] = A_fit
                self.data['A_fit_err'] = A_fit_err
                self.data['phi_fit'] = phi_fit
                self.data['phi_fit_err'] = phi_fit_err
                self.data['dc_dist_array'] = np.concatenate((self.data['dc_dist_array'], np.array([0])))
        ...
        for i in range(self.settings['num_points'] - 1):
            ...
            try:
                A_fit, A_fit_err, phi_fit, phi_fit_err = self.do_dc_sensing(index=i + 1)
                self.data['A_fit'] = np.concatenate((self.data['A_fit'], A_fit))
                self.data['A_fit_err'] = np.concatenate((self.data['A_fit_err'], A_fit_err))
                self.data['phi_fit'] = np.concatenate((self.data['phi_fit'], phi_fit))
                self.data['phi_fit_err'] = np.concatenate((self.data['phi_fit_err'], phi_fit_err))
    ...
    def _plot(self, axes_list, data=None):
        ...
        try:
            ...
            if 'A_fit' in data.keys() and len(data['dc_dist_array']) > 0:
                # axes_list[2].errorbar(data['dc_dist_array'], data['A_fit'], data['A_fit_err'])
                axes_list[2].plot(data['dc_dist_array'], data['A_fit'])
            else:
                axes_list[2].plot(np.zeros([10]), np.zeros([10]))

            if 'phi_fit' in data.keys() and len(data['dc_dist_array']) > 0:
                # axes_list[7].errorbar(data['dc_dist_array'], data['phi_fit'], data['phi_fit_err'])
                axes_list[7].plot(data['dc_dist_array'], data['phi_fit'])
            else:
                axes_list[7].plot(np.zeros([10]), np.zeros([10]))
            ...
            axes_list[0].set_ylabel('Counts [kcps]')
            axes_list[1].set_ylabel('Z_out [V]')
            axes_list[2].set_ylabel('Signal [rad]')
            axes_list[4].set_ylabel('Rabi freq. [MHz]')
            axes_list[5].set_ylabel('RF power [dB]')
            axes_list[7].set_ylabel('Phi [deg]')
            ...
            dc_title = 'Ramsey tau={:2.1f}ns, π/2={:2.1f}ns, detuning: {:0.4f}MHz, Repetition:{:d}\nRF power: {:0.1f}dBm, LO freq: {:0.4f}GHz, IF amp: {:0.1f}, IF freq: {:0.2f}MHz'.format(
                self.scripts['trig_ramsey'].settings['tau'],
                self.scripts['trig_ramsey'].settings['mw_pulses']['pi_half_pulse_time'],
                self.scripts['trig_ramsey'].settings['mw_pulses']['detuning'] * 1e-6,
                self.scripts['trig_ramsey'].settings['rep_num'],
                self.scripts['trig_ramsey'].settings['mw_pulses']['mw_power'],
                self.scripts['trig_ramsey'].settings['mw_pulses']['mw_frequency'] * 1e-9,
                self.scripts['trig_ramsey'].settings['mw_pulses']['IF_amp'],
                self.scripts['trig_ramsey'].settings['mw_pulses']['IF_frequency'] * 1e-6
            )
        ...
        if self._current_subscript_stage['current_subscript'] == self.scripts['trig_ramsey']:
            self.scripts['trig_ramsey']._plot([axes_list[3], axes_list[6]], title=False)
        elif self._current_subscript_stage['current_subscript'] == self.scripts['rabi']:
            self.scripts['rabi']._plot([axes_list[3]], title=False)
        elif self._current_subscript_stage['current_subscript'] == self.scripts['optimize']:
            self.scripts['optimize']._plot([axes_list[3]])
    ...
    def _update_plot(self, axes_list):
        try:
            if self._current_subscript_stage['current_subscript'] == self.scripts['optimize']:
                self.scripts['optimize']._update_plot([axes_list[3]])
            elif self._current_subscript_stage['current_subscript'] == self.scripts['trig_ramsey']:
                self.scripts['trig_ramsey']._update_plot([axes_list[3], axes_list[6]], title=False)
                ...
                if 'A_fit' in self.data.keys() and len(self.data['dc_dist_array']) > 0:
                    axes_list[2].lines[0].set_xdata(self.data['dc_dist_array'])
                    axes_list[2].lines[0].set_ydata(self.data['A_fit'])
                    axes_list[2].relim()
                    axes_list[2].autoscale_view()

                if 'phi_fit' in self.data.keys() and len(self.data['dc_dist_array']) > 0:
                    axes_list[7].lines[0].set_xdata(self.data['dc_dist_array'])
                    axes_list[7].lines[0].set_ydata(self.data['phi_fit'])
                    axes_list[7].relim()
                    axes_list[7].autoscale_view()
...
class ScanningRamseyLockIn2D(Script):
    """
        Perform DC sensing measurements (based on RamseySyncAFM) and perform AFM scan in 2D.
        One can also choose to do Rabi and fluorescence optimize every several points.
        One needs to go to child subscripts to define the settings.
        - Ziwei Qiu 3/28/2021
    """
    _DEFAULT_SETTINGS = [
        ...
        Parameter('scan_size',
                  [Parameter('axis1', 1, float, 'inner loop [V]'),
                   Parameter('axis2', 1, float, 'outer loop [V]')
                   ]),
        Parameter('num_points',
                  [Parameter('axis1', 120, int, 'number of points to scan in each line (inner loop)'),
                   Parameter('axis2', 30, int, 'number of lines to scan (outer loop)'),
                   ]),
        Parameter('scan_speed', 0.02, [0.01, 0.02, 0.04, 0.06, 0.08, 0.1],
                  '[V/s] scanning speed on average. suggest 0.04V/s, i.e. 200nm/s.'),
        Parameter('dc_voltage', [
    ...
    _INSTRUMENTS = {'NI6733': NI6733, 'NI6220': NI6220, 'NI6210': NI6210, 'afg': Agilent33120A,
                    'yokogawa': YokogawaGS200}
    _SCRIPTS = {'scandc1d': ScanningRamseyLockIn, 'set_scanner': SetScannerXY_gentle}
        ...
        if self.settings['to_do'] == 'print_info':
            ...
            for i in range(self.settings['num_points']['axis2']):
                ...
                try:
                    ...
                    A_fit = self.scripts['scandc1d'].data['A_fit']
                    phi_fit = self.scripts['scandc1d'].data['phi_fit']

                    if 'A_fit' in self.data.keys():
                        self.data['A_fit'] = np.concatenate((self.data['A_fit'], A_fit), axis=0)
                    else:
                        self.data['A_fit'] = A_fit
                    if 'phi_fit' in self.data.keys():
                        self.data['phi_fit'] = np.concatenate((self.data['phi_fit'], phi_fit), axis=0)
                    else:
                        self.data['phi_fit'] = phi_fit
...
class ScanningRamsey(Script):
    """
        Perform DC sensing measurements (based on Ramsey at a single tau) and scan along a 1D line.
        One can also choose to do Rabi and fluorescence optimize every several points.
        - Ziwei Qiu 4/22/2021
    """
    _DEFAULT_SETTINGS = [
    ...
    _INSTRUMENTS = {'NI6733': NI6733, 'NI6220': NI6220, 'NI6210': NI6210, 'afg': Agilent33120A,
                    'yokogawa': YokogawaGS200}
    _SCRIPTS = {'ramsey': Ramsey_SingleTau, 'rabi': RabiQM, 'optimize': optimize, 'set_scanner': SetScannerXY_gentle,
                'afm1d': AFM1D_qm, 'afm1d_before_after': AFM1D_qm}
    ...
    def do_dc_sensing(self, label=None, index=-1, verbose=True):
        ...
        if label is not None and index >= 0:
            ...
            self.scripts['ramsey'].settings['tag'] = 'ramsey' + '_ind' + str(index)
        else:
        ...
        if self.settings['do_rabi']['on'] and index >= 0 and index % self.settings['do_rabi'][
            ...
            try:
                ...
                if pi_half_time >= 16:
                    self.scripts['ramsey'].settings['mw_pulses']['mw_frequency'] = \
                        float(self.scripts['rabi'].settings['mw_pulses']['mw_frequency'])
                    self.scripts['ramsey'].settings['mw_pulses']['mw_power'] = \
                        float(self.scripts['rabi'].settings['mw_pulses']['mw_power'])
                    self.scripts['ramsey'].settings['mw_pulses']['IF_frequency'] = \
                        float(self.scripts['rabi'].settings['mw_pulses']['IF_frequency'])
        ...
        if not self._abort:
            ...
            try:
                ...
                print('** ATTENTION in Running ramsey **')
                print(e)
            else:
                dc_sig = self.scripts['ramsey'].data['signal_avg_vec']
                norm_sig = np.array([[2 * (dc_sig[1] - dc_sig[0]) / (dc_sig[1] + dc_sig[0]),
                                      2 * (dc_sig[3] - dc_sig[2]) / (dc_sig[3] + dc_sig[2])]])
                dc_sig = np.array([[dc_sig[0], dc_sig[1], dc_sig[2], dc_sig[3]]])
                exact_tau = np.array([self.scripts['ramsey'].data['tau']])
                coherence = np.sqrt(norm_sig[0, 0] ** 2 + norm_sig[0, 1] ** 2)
                phase = np.arccos(norm_sig[0, 0] / coherence) * np.sign(norm_sig[0, 1])
                coherence = np.array([coherence])
                phase = np.array([phase])
    ...
    def _plot(self, axes_list, data=None):
        ...
        try:
            ...
            dc_title = 'Ramsey: π/2={:2.1f}ns, detuning: {:0.2f}MHz, tau: {:2.1f}ns, Rep:{:d}\nRF power: {:0.1f}dBm, LO freq: {:0.4f}GHz, IF amp: {:0.1f}, IF freq: {:0.2f}MHz'.format(

                self.scripts['ramsey'].settings['mw_pulses']['pi_half_pulse_time'],
                self.scripts['ramsey'].settings['mw_pulses']['detuning'] * 1e-6,
                self.scripts['ramsey'].settings['tau'], self.scripts['ramsey'].settings['rep_num'],
                self.scripts['ramsey'].settings['mw_pulses']['mw_power'],
        ...
        if self._current_subscript_stage['current_subscript'] == self.scripts['ramsey']:
            self.scripts['ramsey']._plot([axes_list[3]], title=False)
        elif self._current_subscript_stage['current_subscript'] == self.scripts['rabi']:
            self.scripts['rabi']._plot([axes_list[3]], title=False)
        elif self._current_subscript_stage['current_subscript'] == self.scripts['optimize']:
            self.scripts['optimize']._plot([axes_list[3]])
    ...
    def _update_plot(self, axes_list):
        try:
            if self._current_subscript_stage['current_subscript'] == self.scripts['optimize']:
                self.scripts['optimize']._update_plot([axes_list[3]])
            elif self._current_subscript_stage['current_subscript'] == self.scripts['ramsey']:
                self.scripts['ramsey']._update_plot([axes_list[3]], title=False)
