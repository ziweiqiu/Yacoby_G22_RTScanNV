...
import numpy as np
from b26_toolkit.instruments import R8SMicrowaveGenerator, NI6602, G22BPulseBlaster
...
class ESR_FastSwp_RnS(Script):
    """
    This class runs ESR on an NV center, outputing microwaves using a Rhode&Schwartz SMB100A RF generator in the frequency
...
class ESR_FastSwp_RnS_FitGuaranteed(Script):
    ...
    def _function(self):
        ...
        # Turn on the microwave switch channel (20200720)
        self.instruments['PB']['instance'].update({'microwave_switch_II': {'status': True}})
...
class ESR_RnS_qm(Script):
    ...
    """
    _DEFAULT_SETTINGS = [
        Parameter('IP_address', '140.247.189.141', ['140.247.189.141', 'automatic'],
    ...
    def _function(self):
        ...
        # Turn on the microwave switch channel (20200720)
        self.instruments['PB']['instance'].update({'mw_switch': {'status': True}})
        ...
        # Turn off the microwave switch channel (20200330)
        self.instruments['PB']['instance'].update({'mw_switch': {'status': False}})
    ...
    def get_axes_layout(self, figure_list):
        ...
        return super(ESR_RnS_qm, self).get_axes_layout(new_figure_list)
...
class CalMagnetAngleQM(Script):
    """

        This script simply calculates the angle between NV and the bias magnetic field by inputting two ESR resonance frequencies.

        --> written by Ziwei Qiu 9/24/2020

        """

    _DEFAULT_SETTINGS = [
        Parameter('LO_freq', 2.87e9, float, 'LO frequency [Hz]'),
        Parameter('esr1', 50.0e6, float, 'first ESR frequency [Hz], relative to LO_freq'),
        Parameter('esr2', 100.0e6, float, 'second ESR frequency [Hz], relative to LO_freq'),
        Parameter('zfs', 2.87E9, float, 'zero-field splitting [Hz]'),
        Parameter('gama', 2.8028E6, float, 'NV spin gyromagnetic ratio [Hz/Gauss]'),
    ]
    ...
    def _function(self):
        ...
        f1 = self.settings['esr1'] + self.settings['LO_freq']
        f2 = self.settings['esr2'] + self.settings['LO_freq']
        B_mag, angle = B_field(f1, f2, self.settings['zfs'], self.settings['gama'])
        self.log('|B| ~ {:.2f} G, θ ~ {:.2f} deg'.format(float(B_mag), float(angle)))
