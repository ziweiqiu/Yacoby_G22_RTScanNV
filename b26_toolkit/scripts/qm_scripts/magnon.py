...
import numpy as np
from b26_toolkit.instruments import SGS100ARFSource, R8SMicrowaveGenerator, AgilentN9310A
...
class ACStarkShiftIQ(Script):
    """
        This script implements AC Stark Shift experiments. Tau is swept.
        pi pulses are generated from SGS100A. Detuned driving is from N9310A.
        - Ziwei Qiu 5/20/2022
    """
    _DEFAULT_SETTINGS = [
        Parameter('IP_address', 'automatic', ['140.247.189.191', 'automatic'],
    ...
    def _function(self):
        ...
        if self.settings['to_do'] == 'reconnection':
            ...
            try:
                ...
                if self.settings['to_do'] == 'simulation':
                    self._qm_simulation(acstark)
                elif self.settings['to_do'] == 'execution':
                    self._qm_execution(acstark, job_stop)
                self._abort = True
    ...
    def plot(self, figure_list):
        super(ACStarkShiftIQ, self).plot([figure_list[0], figure_list[1]])
            ...
            try:
                ...
                axes_list[0].set_title(
                    'AC Stark Shift (type: {:s})\n{:s} {:d} block(s), Ref fluor: {:0.1f}kcps\nRepetition number: {:d}\npi-half time: {:2.1f}ns, pi-time: {:2.1f}ns, 3pi-half time: {:2.1f}ns\nRF power: {:0.1f}dBm, LO freq: {:0.4f}GHz, IF amp: {:0.1f}, IF freq: {:0.2f}MHz\nRF power2: {:0.1f}dBm, LO freq2: {:0.4f}GHz, IF amp2: {:0.1f}, IF freq2: {:0.2f}MHz'.format(
                        ...
                        self.settings['mw_pulses']['IF_amp'],
                        self.settings['mw_pulses']['IF_frequency'] * 1e-6,
                        self.settings['mw_pulses_2']['mw_power'],
                        self.settings['mw_pulses_2']['mw_frequency'] * 1e-9,
                        self.settings['mw_pulses_2']['IF_amp'],
                        self.settings['mw_pulses_2']['IF_frequency'] * 1e-6)
                )
            except Exception as e:
                print('** ATTENTION **')
                print(e)
