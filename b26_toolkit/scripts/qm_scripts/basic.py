...
class RabiQM(Script):
    """
        ...
        - Ziwei Qiu 8/11/2020 last update 5/20/2022
    """
    _DEFAULT_SETTINGS = [
        Parameter('IP_address', 'automatic', ['140.247.189.191', 'automatic'],
    ...
    def _function(self):
        if self.settings['to_do'] == 'reconnection':
            ...
            try:
                ...
                with program() as rabi2:
                    # Use port 9 and 10
                    update_frequency('qubit2', self.settings['mw_pulses']['IF_frequency'])
                    result1 = declare(int, size=res_len)
                    counts1 = declare(int, value=0)
                    result2 = declare(int, size=res_len)
                    counts2 = declare(int, value=0)
                    ...
                    with for_(n, 0, n < rep_num, n + 1):
                        ...
                        with for_each_(t, t_vec):
                            reset_frame('qubit2')
                            z_rot(self.settings['mw_pulses']['phase'] / 180 * np.pi, 'qubit2')
                            play('const' * amp(IF_amp), 'qubit2', duration=t)
                            align('qubit2', 'laser', 'readout1', 'readout2')
                            wait(delay_mw_readout, 'laser', 'readout1', 'readout2')
                            play('trig', 'laser', duration=nv_reset_time)
                            wait(delay_readout, 'readout1', 'readout2')
                            measure('readout', 'readout1', None,
                    ...
                    if title:
                        axes_list[0].set_title(
                            'Rabi frequency: {:0.2f}MHz\npi-half time: {:2.1f}ns, pi-time: {:2.1f}ns, 3pi-half time: {:2.1f}ns \n T2*: {:2.1f}ns, Ref fluor: {:0.1f}kcps\n{}, Repetition: {:d}\nRF power: {:0.1f}dBm, LO freq: {:0.4f}GHz, IF amp: {:0.1f}, IF freq: {:0.2f}MHz'.format(
                                ...
                                data['ref_cnts'], self.settings['generator'], int(data['rep_num']),
...
class ESRQM(Script):
    ...
    _DEFAULT_SETTINGS = [
        ...
        Parameter('to_do', 'simulation', ['simulation', 'execution', 'reconnection'],
                  'choose to do output simulation or real experiment'),
        Parameter('generator', 'SGS100A', ['SGS100A', 'N9310A'], 'choose which generator to use'),
            ...
            try:
                ...
                with program() as ODMR_CW2:
                    ...
                    with for_(n, 0, n < rep_num, n + 1):
                        with for_each_(f, self.f_vec):
                            ...
                            play('const' * amp(IF_amp), 'qubit2', duration=time_per_pt)
                            play('trig', 'laser', duration=time_per_pt)
                            measure('readout', 'readout1', None,
                                    time_tagging.raw(result1, self.meas_len, targetLen=counts1))
                            measure('readout', 'readout2', None,
                ...
                if self.settings['to_do'] == 'simulation':
                    if self.settings['generator'] == 'SGS100A':
                        ...
                        self._qm_simulation(ODMR_CW2)
                elif self.settings['to_do'] == 'execution':
                    if self.settings['generator'] == 'SGS100A':
                        self._qm_execution(ODMR_CW, job_stop)
                    else:
                        self._qm_execution(ODMR_CW2, job_stop)
    ...
    def _qm_execution(self, qua_program, job_stop):
        ...
        if self.settings['generator'] == 'SGS100A':
            generator = 'mw_gen_iq'
            self.instruments[generator]['instance'].update({'ext_trigger': False})
        else:
            generator = 'mw_gen_iq_2'
            self.instruments[generator]['instance'].update({'enable_modulation': True})
            self.instruments[generator]['instance'].update({'freq_mode': 'CW'})
        ...
        self.instruments[generator]['instance'].update({'amplitude': self.settings['power_out']})
        self.instruments[generator]['instance'].update({'frequency': self.settings['mw_frequency']})
        self.instruments[generator]['instance'].update({'enable_IQ': True})
        self.instruments[generator]['instance'].update({'enable_output': True})
        print('Turned on the RF generator (IQ on, trigger on).')
...
class ESRQM_FitGuaranteed(Script):
    """
        This class runs ESR on an NV center. MW frequency is swept by sweeping the IF frequency output by the QM.
        - Ziwei Qiu 9/18/2020, last update 5/20/2022
    """
    _DEFAULT_SETTINGS = [
        Parameter('IP_address', 'automatic', ['140.247.189.191', 'automatic'],
                  'IP address of the QM server'),
