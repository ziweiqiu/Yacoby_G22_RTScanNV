...
class PulseBlaster(Instrument):
    ...
    def update(self, settings):
        ...
        # (ZQ 20190712 for the RT scanning setup)
        ...
        # (ZQ 20190513 for the 6th floor setup) To address channel number > 20, remove the three 1's in the beginning.
        #  Checked it still works.
        ...
        # self.pb.pb_stop() # comment this to keep output static ZQ
...
class G22BPulseBlaster(PulseBlaster):
    """
        This Instrument controls the SpinCore Pulseblaster used in the LISE G22B NV-RT Scanning Setup.
        The internal clock speed is 400MHz.
        Notice that the PB channels number labeled here is smaller than that in the MATLAB software by 1 due to the
        stupid difference between MATLAB and Python
        - Ziwei Qiu 7/12/2020
    """
    _DEFAULT_SETTINGS = Parameter([
        ...
        Parameter('mw_switch', [
            Parameter('channel', 1, int, 'channel to which AWG trigger input is connected'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),
        Parameter('apd_readout', [
            Parameter('channel', 2, int, 'channel to which the counter1 daq is connected to (for gated counter)'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),
        ...
        Parameter('channel_PB6', [
            Parameter('channel', 6, int, 'nothing is connected'),
            Parameter('status', False, bool, 'True if voltage is high to the trig_channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),

        Parameter('channel_PB7', [
            Parameter('channel', 7, int, 'nothing is connected'),
            Parameter('status', False, bool, 'True if voltage is high to the RF switch, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and RF switch [ns]')
        ]),

        Parameter('channel_PB14', [
            Parameter('channel', 14, int, 'channel to which the NI DAQ is connected'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),

        Parameter('clock_speed', 400, [100, 400, 500], 'Clock speed of the pulse blaster [MHz]'),
        Parameter('min_pulse_dur', 15, [2, 5, 10, 15, 50], 'Minimum allowed pulse duration (ns)'),
        Parameter('PB_type', 'PCI', ['PCI', 'USB'], 'Type of pulseblaster used'),
    ...
    def __init__(self, name=None, settings=None):
        # COMMENT_ME
        super(G22BPulseBlaster, self).__init__(name, settings)

    def update(self, settings):
        # call the update_parameter_list to update the parameter list
        # oh god this is confusing
        # print('update PB')
        super(G22BPulseBlaster, self).update(settings)
