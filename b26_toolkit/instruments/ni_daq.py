...
class DAQ(Instrument):
    ...
    def setup_counter(self, channel, sample_num, continuous_acquisition=False):
        ...
        # set up counter using clock as reference (specify the source of the sample clock)
        self._check_error(self.nidaq.DAQmxCreateTask("", ctypes.byref(task['task_handle_ctr'])))
        # http://zone.ni.com/reference/en-XX/help/370471AM-01/daqmxcfunc/daqmxcreatecopulsechanfreq/
    ...
    def setup_AO(self, channels, waveform, clk_source=""):
        ...
        # print('task sample_rate', task['sample_rate'] )
        ...
        # print('int32 numSampsPerChan:', int32(task['sample_num']))
    ...
    # todo: AK - should this be threaded?
    # original todo: is this actually blocking? Is the threading actually doing anything? see nidaq cookbook
    ...
    def get_analog_voltages(self, channel_list):
        ...
        for channel in channel_list:
            ...
            #     print('this is an analog output channel')
            #     daq_channels_str += self.settings['device'] + '/_' + channel + '_vs_aognd, '
            # elif (channel in self.settings['analog_input']):
            #     print('this is an analog input channel')
            #     daq_channels_str += self.settings['device'] + '/' + channel + ', '

            if (channel in self.settings['analog_input']):
                # print('this is an analog input channel')
                daq_channels_str += self.settings['device'] + '/' + channel + ', '
                # daq_channels_str += self.settings['device'] + '/' + channel + '_vs_aignd, ' # Ziwei Qiu 12/7/20
            else:
                # print('this is an analog output channel')
                daq_channels_str += self.settings['device'] + '/_' + channel + '_vs_aognd, '
...
class NI6733(DAQ):
    """
    This class implements the NI6733 DAQ, which includes 8 AO channels.
    It inherits output functionality from the DAQ class. (Used in G22B RT scanning setup)
    - ZQ 7/13/2020
    """
    _DEFAULT_SETTINGS = Parameter([
        Parameter('device', 'PXI1Slot3', ['PXI1Slot3'], 'Name of DAQ device - check in NiMax'),
        Parameter('override_buffer_size', -1, int, 'Buffer size for manual override (unused if -1)'),
        Parameter('ao_read_offset', 0, float, 'Empirically determined offset for reading ao voltages internally'),
        Parameter('analog_output',
                  [
                      Parameter('ao0',
                                [
                                    Parameter('channel', 0, [0, 1, 2, 3, 4, 5, 6, 7], 'output channel'),
                                    Parameter('sample_rate', 1000.0, float, 'output sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum output voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum output voltage (V)')
                                ]
                      ...
                      Parameter('ao1',
                                [
                                    Parameter('channel', 1, [0, 1, 2, 3, 4, 5, 6, 7], 'output channel'),
                                    Parameter('sample_rate', 1000.0, float, 'output sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum output voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum output voltage (V)')
                                ]
                      ...
                      Parameter('ao2',
                                [
                                    Parameter('channel', 2, [0, 1, 2, 3, 4, 5, 6, 7], 'output channel'),
                                    Parameter('sample_rate', 1000.0, float, 'output sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum output voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum output voltage (V)')
                                ]
                      ...
                      Parameter('ao3',
                                [
                                    Parameter('channel', 3, [0, 1, 2, 3, 4, 5, 6, 7], 'output channel'),
                                    Parameter('sample_rate', 1000.0, float, 'output sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum output voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum output voltage (V)')
                                ]
                      ...
                      Parameter('ao4',
                                [
                                    Parameter('channel', 4, [0, 1, 2, 3, 4, 5, 6, 7], 'output channel'),
                                    Parameter('sample_rate', 1000.0, float, 'output sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum output voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum output voltage (V)')
                                ]
                      ...
                      Parameter('ao5',
                                [
                                    Parameter('channel', 5, [0, 1, 2, 3, 4, 5, 6, 7], 'output channel'),
                                    Parameter('sample_rate', 1000.0, float, 'output sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum output voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum output voltage (V)')
                                ]
                      ...
                      Parameter('ao6',
                                [
                                    Parameter('channel', 6, [0, 1, 2, 3, 4, 5, 6, 7], 'output channel'),
                                    Parameter('sample_rate', 1000.0, float, 'output sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum output voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum output voltage (V)')
                                ]
                      ...
                      Parameter('ao7',
                                [
                                    Parameter('channel', 7, [0, 1, 2, 3, 4, 5, 6, 7], 'output channel'),
                                    Parameter('sample_rate', 1000.0, float, 'output sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum output voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum output voltage (V)')
                                ]
...
class NI6602(DAQ):
    """
    This class implements the NI PCI-6602 DAQ, which is a 5 V, 8-Channel Counter/Timer Device.
    Currently used in G22B RT scanning setup.
    https://www.ni.com/pdf/manuals/372141b.pdf
    https://neurophysics.ucsd.edu/Manuals/National%20Instruments/6601_6602%20User%20Manual.pdf
    - updated by ZQ 7/13/2020
    """
    _DEFAULT_SETTINGS = Parameter([
        Parameter('device', 'PXI1Slot4', ['PXI1Slot4'], 'Name of DAQ device - check in NiMax'),
        Parameter('override_buffer_size', -1, int, 'Buffer size for manual override (unused if -1)'),
        Parameter('digital_input',
                  [
                      Parameter('ctr0',
                                [
                                    Parameter('input_channel', 0, list(range(0, 40)), 'channel for counter signal input'),
                                    Parameter('counter_PFI_channel', 39, list(range(0, 40)), 'PFI for counter channel input (SRC)'),
                                    Parameter('gate_PFI_channel', 38, list(range(0, 40)), 'PFI for counter channel input (GATE)'),
                                    Parameter('clock_PFI_channel', 24, list(range(0, 40)), 'PFI for the source of the sample clock'),
                                    Parameter('clock_counter_channel', 3, int, 'the ctr channel created for generating pulses'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)')
                                ]
                                ),
                      Parameter('ctr1',
                                [
                                    Parameter('input_channel', 1, list(range(0, 40)), 'channel for counter signal input'),
                                    Parameter('counter_PFI_channel', 35, list(range(0, 40)), 'PFI for counter channel input (SRC)'),
                                    Parameter('gate_PFI_channel', 34, list(range(0, 40)), 'PFI for counter channel input (GATE)'),
                                    Parameter('clock_PFI_channel', 24, list(range(0, 40)), 'PFI for clock channel output (OUT)'),
                                    Parameter('clock_counter_channel', 3, int, 'channel for clock output'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)')
                                ]
                                )
                  ]
...
class NI6220(DAQ):
    """
    This class implements the NI6220 DAQ, which includes 16 AO channels. It inherits output functionality from the DAQ
    class.
    """
    _DEFAULT_SETTINGS = Parameter([
        Parameter('device', 'PXI1Slot5', ['PXI1Slot5'],
                  'Name of DAQ device - check in NiMax'),
        Parameter('override_buffer_size', -1, int, 'Buffer size for manual override (unused if -1)'),
        Parameter('ao_read_offset', 0, float, 'Empirically determined offset for reading ao voltages internally'),
        Parameter('analog_input',
                  [
                      Parameter('ai0',
                                [
                                    Parameter('channel', 0, [0], 'input channel'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum input voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum input voltage (V)')
                                ]
                      ...
                      Parameter('ai1',
                                [
                                    Parameter('channel', 1, [1], 'input channel'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum input voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum input voltage (V)')
                                ]
                      ...
                      Parameter('ai2',
                                [
                                    Parameter('channel', 2, [2], 'input channel'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum input voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum input voltage (V)')
                                ]
                      ...
                      Parameter('ai3',
                                [
                                    Parameter('channel', 3, [3], 'input channel'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum input voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum input voltage (V)')
                                ]
                      ...
                      Parameter('ai4',
                                [
                                    Parameter('channel', 4, [4], 'input channel'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum input voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum input voltage (V)')
                                ]
                      ...
                      Parameter('ai5',
                                [
                                    Parameter('channel', 5, [5], 'input channel'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum input voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum input voltage (V)')
                                ]
                      ...
                      Parameter('ai6',
                                [
                                    Parameter('channel', 6, [6], 'input channel'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum input voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum input voltage (V)')
                                ]
                      ...
                      Parameter('ai7',
                                [
                                    Parameter('channel', 7, [7], 'input channel'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum input voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum input voltage (V)')
                                ]
                      ...
                      Parameter('ai8',
                                [
                                    Parameter('channel', 8, [8], 'input channel'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum input voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum input voltage (V)')
                                ]
                      ...
                      Parameter('ai9',
                                [
                                    Parameter('channel', 9, [9], 'input channel'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum input voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum input voltage (V)')
                                ]
                      ...
                      Parameter('ai10',
                                [
                                    Parameter('channel', 10, [10], 'input channel'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum input voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum input voltage (V)')
                                ]
                      ...
                      Parameter('ai11',
                                [
                                    Parameter('channel', 11, [11], 'input channel'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum input voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum input voltage (V)')
                                ]
                      ...
                      Parameter('ai12',
                                [
                                    Parameter('channel', 12, [12], 'input channel'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum input voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum input voltage (V)')
                                ]
                      ...
                      Parameter('ai13',
                                [
                                    Parameter('channel', 13, [13], 'input channel'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum input voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum input voltage (V)')
                                ]
                      ...
                      Parameter('ai14',
                                [
                                    Parameter('channel', 14, [14], 'input channel'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum input voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum input voltage (V)')
                                ]
                      ...
                      Parameter('ai15',
                                [
                                    Parameter('channel', 15, [15], 'input channel'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)'),
                                    Parameter('min_voltage', -10.0, float, 'minimum input voltage (V)'),
                                    Parameter('max_voltage', 10.0, float, 'maximum input voltage (V)')
                                ]
    ...
    _DEFAULT_SETTINGS = Parameter([
        ...
        Parameter('digital_input',
                  [
                      Parameter('ctr0',
                                [
                                    Parameter('input_channel', 0, list(range(0, 5)),
                                              'channel for counter signal input'),
                                    Parameter('counter_PFI_channel', 0, list(range(0, 5)),
                                              'PFI for counter channel input (SRC)'),
                                    Parameter('gate_PFI_channel', 1, list(range(0, 5)),
                                              'PFI for counter channel input (GATE)'),
                                    Parameter('clock_PFI_channel', 4, list(range(0, 5)),
                                              'PFI for clock channel output (OUT)'),
                                    Parameter('clock_counter_channel', 0, [0, 1], 'channel for clock output'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)')
                                ]
                                ),
                      Parameter('ctr1',
                                [
                                    Parameter('input_channel', 1, list(range(0, 8)),
                                              'channel for counter signal input'),
                                    Parameter('counter_PFI_channel', 3, list(range(0, 8)),
                                              'PFI for counter channel input (SRC)'),
                                    Parameter('gate_PFI_channel', 2, list(range(0, 8)),
                                              'PFI for counter channel input (GATE)'),
                                    Parameter('clock_PFI_channel', 5, list(range(0, 8)),
                                              'PFI for clock channel output (OUT)'),
                                    Parameter('clock_counter_channel', 1, [0, 1], 'channel for clock output'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)')
                                ]
        ...
        Parameter('digital_output',
                  [
                      Parameter('do0',
                                [
                                    Parameter('channel', 4, list(range(0, 8)), 'channel'),
                                    Parameter('sample_rate', 1000.0, float, 'output sample rate (Hz)')
                      ...
                      Parameter('do1',
                                [
                                    Parameter('channel', 5, list(range(0, 8)), 'channel'),
                                    Parameter('sample_rate', 1000.0, float, 'output sample rate (Hz)')
                      ...
                      Parameter('do2',
                                [
                                    Parameter('channel', 6, list(range(0, 8)), 'channel'),
                                    Parameter('sample_rate', 1000.0, float, 'output sample rate (Hz)')
