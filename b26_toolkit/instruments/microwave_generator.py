...
class R8SMicrowaveGenerator(Instrument):
    ...
    Modified VISA Address on the scanning setup

    -Ziwei (7/10/2020 5:50pm)
    """

    _DEFAULT_SETTINGS = Parameter([
        Parameter('VISA_address','GPIB0::28::INSTR',['GPIB0::28::INSTR'],'VISA address of the instrument'),
...
class SGS100ARFSource(Instrument):
    """
        This class implements the ROHDE & SCHWARZ RF Source SGS100A. The class commuicates with the
        device over USB or GPIB using pyvisa.
        Manual: https://micro-electronics.ru/upload/iblock/4f3/4f31ef667b3f4ecee2058f6c1e7c4d1c.pdf
        -Ziwei 7/15/2020
    """

    _DEFAULT_SETTINGS = Parameter([
        Parameter('VISA_address', 'USB0::0x0AAD::0x0088::110207::0::INSTR', ['USB0::0x0AAD::0x0088::110207::0::INSTR'],
                  'VISA address of the instrument'),
        Parameter('frequency', 2.87e9, float, 'RF frequency in Hz, range: 1000kHz to 6GHz'),
        Parameter('amplitude', -50, float, 'RF power in dBm, range: -127 to +20dBm'),
        Parameter('enable_output', False, bool, 'RF output enabled'),
        Parameter('enable_IQ', False, bool, 'enable IQ modulation'),
        Parameter('ext_trigger', False, bool, 'expects pulse modulation signals at the TRIG connector'),
    ])

    def __init__(self, name=None, settings=None):

        super(SGS100ARFSource, self).__init__(name, settings)
    ...
    def _connect(self, verbose=False):
        ...
        # No external reference oscillator is present now. So only use the internal one.
        self.srs.write(':SOURce:ROSCillator:SOURce INTernal')  # Enable internal reference frequency source
        self.srs.write(':SOURce:LOSCillator:SOURce INT')  #Enable internal LO source
        self.srs.write(':SOURce:PULM:SOURce EXT') #The signal applied externally via the trigger connector is used for the pulse modulation.
        self.srs.write(':SOURce:PULM:POL NORM') # The RF signal is suppressed during the pulse pause (low), and is enabled at high pulse
        self.srs.write(':PULM:TRIGger:EXTernal:IMPedance G50')

        if verbose:
            print('SGS100A reference oscillator:' + self.srs.query(':SOURce:ROSCillator:SOURce?'))
            print('SGS100A local oscillator:' + self.srs.query(':SOURce:LOSCillator:SOURce?'))
            print('SGS100A pulse modulation source:' + self.srs.query(':SOURce:PULM:SOURce?'))
            print('SGS100A pulse polarity:' + self.srs.query(':SOURce:PULM:POL?'))
            print('SGS100A external pulse trigger impedance:' + self.srs.query(':PULM:TRIGger:EXTernal:IMPedance?'))
    ...
    def update(self, settings):
        ...
        """
        # print('Rhode Schwartz start updating')
        super(SGS100ARFSource, self).update(settings)
        # XXXXX MW ISSUE = START
        # ===========================================
        RANGE_MIN = 1000 # 1MHz
        RANGE_MAX = 6000000000  # 6 GHZ
        PWR_MIN = -120 # -120dBm
        PWR_MAX = 25 # 25 dBm
        if self._settings_initialized:
            for key, value in settings.items():
                ...
                #         self.srs.write(':SOURce:ROSCillator:SOURce INTernal') #Enable internal reference frequency source
                    # elif value == 'EXT10MHZ':
                    #     self.srs.write(':SOURce:ROSCillator:SOURce EXTernal') #Enable internal reference frequency source
                    #     self.srs.write(':SOURce:ROSCillator:EXTernal:FREQuency 10MHZ')
                else:
                    # print('current key', key)
                    if key == 'enable_output' or key == 'enable_IQ' or key == 'ext_trigger':
                        value = self._output_to_internal(value)
                    elif key == 'frequency':
                        if value > RANGE_MAX or value < RANGE_MIN:
                            print("Invalid frequency. All frequencies must be between 1 MHz and 6 GHz.")
                    elif key == 'amplitude':
                        if value > PWR_MAX or value < PWR_MIN:
                            print("Invalid power. All powers must be between -120dBm and 25dBm.")
    ...
    def _param_to_internal(self, param):
        ...
        if param == 'enable_output':
            ...
            return ':SOURce:FREQuency:CW'
        elif param == 'amplitude':
            return ':SOURce:POWer'
        elif param == 'enable_IQ':
            return ':SOURce:IQ:STATe'
        elif param == 'ext_trigger':
            return ':SOURce:PULM:STATe'
        else:
            raise KeyError
...
class RnSMicrowaveGenerator(Instrument):
    ...
    def _connect(self):
        ...
        self.srs = rm.open_resource(self.settings['VISA_address'])
        self.srs.query('*IDN?')
        print('SMB100A connected: '+  self.srs.query('*IDN?'))
